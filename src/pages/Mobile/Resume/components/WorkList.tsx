...
const WorkExperienceList: React.FC<WorkExperienceListProps> = (props) => {
  ...
  const simpleHelp = (index: number, currentField: string) =>
    customFormListHelp(errors, 'workExperienceList', index, currentField);

  const simpleRules = (index: number, currentField: string) => [
    {
      validator: async (_: any, value: any) =>
        customListValidator(setErrors, 'workExperienceList', index, currentField, value, true),
    },
  ];
  ...
  return (
    <>
      <Form.List
        ...
        rules={[
          {
            validator: async (_: any, workExperienceList: any) =>
              validateWorkExperienceList(workExperienceList, setErrors),
          },
        ]}
      >
        {(fields, { add, remove }) => (
              ...
              <Collapse defaultActiveKey={field.name} key={field.name}>
                ...
                >
                  <ZaCellInput
                    ...
                    required={true}
                    help={simpleHelp(index, 'workCompany')}
                    rules={simpleRules(index, 'workCompany')}
                  />
                  <ZaCellDataSelect
